...
for d in dir_names:
	...
	for ld in lc_dirs :
		...
		for f in lc_files :
			...
			output_name = '/'.join(f.split('/')[:]) + '/ref.cat'
			param_name = '/'.join(f.split('/')[:]) + '/star_params.dat'

			# if 'on' in f : fits_name = ('/'.join(f.split('/')[:-1]) + '.fits')
			# print(f , fits_name , output_name)
			# else: continue
			# if True: break
			...
			try: 
				star_id , ra , dec , s , L , A , b , x , y , a , flux , X_0 , Y_0 = np.loadtxt ( param_name , skiprows=1 , unpack=True )
				print(f'{len(star_id)} stars ')
			...
			args_str = f'./refcat {np.mean(ra_dec.ra.deg)} {np.mean(ra_dec.dec.deg)} -rad .5 -dir "/run/media/mehul/newhope/00_m_16/" -all'
			args_str1 = f'./refcat {np.mean(ra_dec.ra.deg)} {np.mean(ra_dec.dec.deg)} -rad .5 -dir "/run/media/mehul/newhope/16_m_17/" -all'
			args_str2 = f'./refcat {np.mean(ra_dec.ra.deg)} {np.mean(ra_dec.dec.deg)} -rad .5 -dir "/run/media/mehul/newhope/17_m_18/" -all'
			args_str3 = f'./refcat {np.mean(ra_dec.ra.deg)} {np.mean(ra_dec.dec.deg)} -rad .5 -dir "/run/media/mehul/newhope/18_m_19/" -all -mlim 20'
			args_str4 = f'./refcat {np.mean(ra_dec.ra.deg)} {np.mean(ra_dec.dec.deg)} -rad .5 -dir "/run/media/mehul/newhope/19_m_20/" -all -mlim 20'
